  ...
  function addCustomFields(loggingEvent, msg) {
    ...
    Object.keys(defaultCustomFields).forEach(function(key) {
      ...
      if (key.match(/^_/) && key !== "_id") {
        if (typeof(defaultCustomFields[key]) !== 'undefined') {
          if (typeof(defaultCustomFields[key]) === 'function') {
            msg[key] = defaultCustomFields[key](loggingEvent);
          } else {
    ...
    if (firstData) { //if there is data we can append custom file
