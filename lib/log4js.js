...
function configureAppenders(appenderList, fileAppender, consoleAppender, smtpAppender) {
    clearAppenders();
    if (appenderList) {
	appenderList.forEach(function(appenderConfig) {
	    var appender = appenderMakers[appenderConfig.type](appenderConfig, fileAppender, consoleAppender, smtpAppender);
	    if (appender) {
...
module.exports = function (fileSystem, standardOutput, configPaths) {
    ...
    thismodule.connectLogger = require(__dirname + '/connect-logger')(thismodule).connectLogger;

	// load extensions
    var extensions = fs.readdirSync(__dirname + '/extensions');
    extensions.forEach(function(extension){
    	// boot extension
    	try {
			extension=require(__dirname + '/extensions/' + extension)(thismodule);
			
			// merge log4js with extensions' exports
			for (var expo in extension)
			if (extension.hasOwnProperty(expo)) {
				thismodule[expo] = extension[expo];
			}
		} catch(err){
			// meh...
		}
