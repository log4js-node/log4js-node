...
function initReloadConfiguration(filename, options) {
  ...
  configState.expires = getExpires(fileName);
}
...
function purge(appender) {
  var appenderModule = require('./appenders/' + appender);
  try {
    var msg = appenderModule.purge();
    if (msg.length > 0) {
      getLogger('log4js').info(
        'Purged expired log files: \n' + msg
      );
    }
  } catch (e) {
    getLogger('log4js').warn(
      'Error while trying to purge a non-purgable appender: ', e
    );
  }
}
/**
