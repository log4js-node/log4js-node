...
function parseStack(data, skipIdx = 4) {
...
class Logger {
  ...
  _log(level, data) {
    ...
    if (this._useStack) {
      const location = parseStack(new Error());
      loggingEvent = new LoggingEvent(this.category, level, data, this.context, location);
    } else {
  ...
  enableCallStack(bool = true) {
    this._useStack = (bool === true);
    return this;
  }

  isCallStackEnable() {
    return this._useStack === true;
  }
}
