...
const DEFAULT_FORMAT = ':remote-addr - -'
  ...
  + ' ":user-agent"'
  + ' ":body"';
const REQUIRED_BODY_METHOD = ['POST', 'PATCH', 'PUT'];
...
function getBody(req) {
  if (REQUIRED_BODY_METHOD.includes(req.method)) {
...
function assembleTokens(req, res, customTokens) {
  ...
  const accessCustomTokens = (req, res, cts) => {
    const array = [];

    for (const ct of cts) {
      array.push({
        token: ct.token,
        replacement: (typeof ct.replacement === 'function') ? ct.replacement(req, res) : ct.replacement
      })
    }
  ...
  defaultTokens.push({
    token: ':body',
    replacement: getBody(req)
  });

  const accessedTokens = accessCustomTokens(req, res, customTokens);

  return arrayUniqueTokens(accessedTokens.concat(defaultTokens));
}
