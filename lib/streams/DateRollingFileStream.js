...
function DateRollingFileStream(filename, pattern, options, now) {
  ...
  this.backups = options && options.backups;
...
DateRollingFileStream.prototype.roll = function(filename, callback) {
  ...
  function deleteExcessBackupFile(cb) {
    if (!that.backups) {
      cb();
      return;
    }
    var now = new Date(that.now());
    now.setDate(now.getDate() - that.backups - 1);
    var deleteFileName = that.baseFilename + format.asString(that.pattern, now);
    debug("Deleting the " + deleteFileName);
