...
function inheritFromParent(config, category, categoryName) {
  ...
  if (!parentCategory) {
    // parent is missing, may need to implicitly create it, so that it can inherit from its parents
    if (parentCategoryName.lastIndexOf('.') >= 0) { // don't create a missing root category, it can inherit nothing
